...
maxT = 40
numSims = 100
treatmentsAvailable = 100

gammaHats = np.linspace(0.3, 1.0, 15)
...
for gammaHat in gammaHats:
    ...
    for n in range(0, numSims):
        ...
        for t in range(1, maxT + 1):
            ...
            sortedPotentialTreatmentTakers = sortedSubsetofPopulation(population, [label for label in potentialTreatmentTakers if population[label].state == "I"], "infectiousness")
        ...
        for i in range(0, N):
            if (population[i].state == "I"):
                print("ERROR: Infection did not die out when gammaHat = " + str(gammaHat) + ". Try increasing maxT.")

        #Store fatality numbers for this run
        listofDs.append(D)
        print("Completed simulation " + str(n) + " for gammaHat = " + str(gammaHat) + ".")
...
averageFinalDs = np.zeros(len(gammaHats))
stdDevFinalDs = np.zeros(len(gammaHats))
for k in range(0, len(gammaHats)):
...
plt.plot(gammaHats, averageFinalDs, 'k-', linewidth = 2.5)
plt.fill_between(gammaHats, averageFinalDs - stdDevFinalDs, averageFinalDs + stdDevFinalDs, color = 'blue', alpha = 0.3)
plt.xlim([0.3, 1.0])
plt.ylim([0, 250])
